...
im_data_s = torch.FloatTensor(next(iter(source_loader))[0].size())
im_data_t = torch.FloatTensor(next(iter(target_loader))[0].size())
im_data_tu = torch.FloatTensor(next(iter(target_loader_unl))[0].size())
gt_labels_s = torch.LongTensor(next(iter(source_loader))[1].size())
gt_labels_t = torch.LongTensor(next(iter(target_loader))[1].size())
...
def train():
    ...
    for step in range(all_step):
        ...
        if step % args.save_interval == 0 and step > 0:
            loss_test, acc_test, fscore_test = test(target_loader_test)
            ...
            if fscore_val >= best_fscore:
                best_fscore = fscore_val
                best_fscore_test = fscore_test
                counter = 0
            else:
                counter += 1
            if args.early:
            ...
            print('best fscore test %f best fscore val %f' % (best_fscore_test,
                                                        fscore_val))
