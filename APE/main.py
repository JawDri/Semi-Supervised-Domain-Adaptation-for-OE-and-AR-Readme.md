...
parser.add_argument('--steps', type=int, default=520, metavar='N',
...
def train():
    ...
    for step in range(all_step):
        ...
        # attraction scheme
        #print(latent_F1.shape, latent_F1_tu.shape)
        ...
        #print(perturb_logits.shape, clean_vat_logits.shape)
        ...
        if step % args.save_interval == 0 and step > 0:
            loss_test, acc_test, fscore_test = test(G, F1, target_loader_test)
            #loss_val, acc_val, fscore_val = test(G, F1, target_loader_val)
            G.train()
            F1.train()
